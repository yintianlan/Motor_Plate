...
//========================================================================
// 函数: void	PWM_UpdatePwm(void)
// 描述: 更新PWM占空比. 
// 参数: none.
// 返回: none.
//========================================================================
void PWM_UpdatePwm(void)
{
	unsigned short PWM1_Duty = CYCLE / 2;
	unsigned short PWM2_Duty = CYCLE / 4;
	unsigned short PWM3_Duty = CYCLE - 1;
	unsigned short PWM4_Duty = 0;
	
	P_SW2 |= 0x80;      //使能访问PWM在扩展RAM区的特殊功能寄存器XSFR
	
	PWMB_CCR5H = (unsigned char)(PWM1_Duty >> 8); //设置占空比时间
    PWMB_CCR5L = (unsigned char)(PWM1_Duty);
	
	PWMB_CCR6H = (unsigned char)(PWM2_Duty >> 8); //设置占空比时间
    PWMB_CCR6L = (unsigned char)(PWM2_Duty);
	
	PWMB_CCR7H = (unsigned char)(PWM3_Duty >> 8); //设置占空比时间
    PWMB_CCR7L = (unsigned char)(PWM3_Duty);
	
	PWMB_CCR8H = (unsigned char)(PWM4_Duty >> 8); //设置占空比时间
    PWMB_CCR8L = (unsigned char)(PWM4_Duty);
